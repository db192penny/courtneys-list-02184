-- Update list_vendor_stats function to count vendor creators as users
...
AS $function$
  ...
  creator_households AS (
    SELECT v.id as vendor_id, count(DISTINCT hh.household_address)::int AS homes
  ...
  homes AS (
    ...
    FROM (
      ...
      UNION ALL
      SELECT * FROM creator_households
    ) s
    GROUP BY vendor_id
  ),
  hoa_reviews AS (
