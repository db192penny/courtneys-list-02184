-- Create vendor_market_prices table for admin-editable baseline pricing
CREATE TABLE IF NOT EXISTS public.vendor_market_prices (
  vendor_id uuid PRIMARY KEY REFERENCES public.vendors(id) ON DELETE CASCADE,
  amount numeric NOT NULL CHECK (amount > 0),
  unit text NOT NULL CHECK (unit IN ('month', 'visit', 'hour', 'job')),
  updated_by uuid NOT NULL,
  updated_at timestamptz NOT NULL DEFAULT now()
);

-- Enable RLS
ALTER TABLE public.vendor_market_prices ENABLE ROW LEVEL SECURITY;

-- RLS Policies
DROP POLICY IF EXISTS "Read market prices" ON public.vendor_market_prices;
CREATE POLICY "Read market prices"
  ON public.vendor_market_prices
  FOR SELECT
  TO authenticated
  USING (true);

DROP POLICY IF EXISTS "Admins manage market prices" ON public.vendor_market_prices;
...
-- Update list_vendor_stats function to include community mode and market pricing
