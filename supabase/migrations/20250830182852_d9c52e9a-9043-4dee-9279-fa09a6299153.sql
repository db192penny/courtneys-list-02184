-- Update admin_soft_delete_user function to handle simple_invites table
...
BEGIN
  ...
  -- Delete from simple_invites table (both as inviter and invitee)
  DELETE FROM public.simple_invites WHERE inviter_id = _user_id OR used_by = _user_id;
  
  -- Delete from new invite system tables
  DELETE FROM public.invite_redemptions WHERE inviter_id = _user_id OR invited_user_id = _user_id;
  DELETE FROM public.invite_codes WHERE user_id = _user_id;
  
  -- Delete from legacy invitations table if it exists
