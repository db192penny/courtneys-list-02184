-- Fix the admin_soft_delete_user function to handle missing invitations_legacy table
...
BEGIN
  ...
  -- Only delete from legacy tables if they exist (use DO block for conditional execution)
  DO $$
  BEGIN
    IF EXISTS (SELECT FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'invitations_legacy') THEN
