-- Update list_vendor_stats function to include addresses from reviews in homes count
CREATE OR REPLACE FUNCTION public.list_vendor_stats(_hoa_name text, _category text DEFAULT NULL::text, _sort_by text DEFAULT 'homes'::text, _limit integer DEFAULT 100, _offset integer DEFAULT 0)
 RETURNS TABLE(id uuid, name text, category text, homes_serviced integer, homes_pct numeric, hoa_rating numeric, hoa_rating_count integer, google_rating numeric, google_rating_count integer, avg_monthly_cost numeric, monthly_sample_size integer, service_call_avg numeric, service_call_sample_size integer, contact_info text, typical_cost numeric, avg_cost_display text, avg_cost_amount numeric)
 LANGUAGE sql
...
AS $function$
  ...
  -- Get all distinct addresses that have interacted with each vendor, excluding admin creators
  homes AS (
    SELECT vendor_id, count(DISTINCT normalized_address)::int AS homes
    FROM (
      -- Addresses from costs
      ...
      -- Addresses from vendor creators (EXCLUDING admins)
      ...
      JOIN public.household_hoa hh ON hh.household_address = public.normalize_address(u.address)
      WHERE lower(hh.hoa_name) = lower(trim(_hoa_name))
        AND NOT public.has_role(v.created_by, 'admin'::public.app_role)  -- Exclude admin creators
      
      UNION
      
      -- Addresses from reviews (users who rated vendors)
  ...
  SELECT
    ...
    v.contact_info,
    v.typical_cost,
    -- Category-specific cost display logic
    CASE 
    ...
    -- Category-specific cost amount - FIXED to include Power Washing
    CASE 
