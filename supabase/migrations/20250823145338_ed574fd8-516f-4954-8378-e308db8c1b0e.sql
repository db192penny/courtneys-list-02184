-- Update list_vendor_stats function to include preview_reviews in HOA rating calculations
CREATE OR REPLACE FUNCTION public.list_vendor_stats(_hoa_name text, _category text DEFAULT NULL::text, _sort_by text DEFAULT 'homes'::text, _limit integer DEFAULT 100, _offset integer DEFAULT 0)
 RETURNS TABLE(id uuid, name text, category text, homes_serviced integer, homes_pct numeric, hoa_rating numeric, hoa_rating_count integer, google_rating numeric, google_rating_count integer, google_reviews_json jsonb, google_place_id text, avg_monthly_cost numeric, monthly_sample_size integer, service_call_avg numeric, service_call_sample_size integer, contact_info text, typical_cost numeric, avg_cost_display text, avg_cost_amount numeric, community_amount numeric, community_unit text, community_sample_size integer, market_amount numeric, market_unit text)
 LANGUAGE sql
...
AS $function$
  ...
  households AS (
    SELECT normalized_address FROM public.household_hoa
    WHERE lower(hoa_name) = lower(trim(_hoa_name))
  ),
  homes AS (
    SELECT vendor_id, count(DISTINCT normalized_address)::int AS homes
    FROM (
      -- Reviews: count homes where residents reviewed the vendor
      ...
      -- Current home vendors: count homes that currently use the vendor
  ...
  hoa_reviews AS (
    SELECT vendor_id,
           avg(rating)::numeric(4,2) AS hoa_rating,
           count(*)::int AS hoa_rating_count
    FROM (
      -- Include regular reviews from registered users
      SELECT r.vendor_id, r.rating
      FROM public.reviews r
      JOIN public.users u ON u.id = r.user_id
      JOIN public.household_hoa hh ON hh.normalized_address = public.normalize_address(u.address)
      WHERE lower(hh.hoa_name) = lower(trim(_hoa_name))
      ...
      -- Include preview reviews from community sessions
      SELECT pr.vendor_id, pr.rating
      FROM public.preview_reviews pr
      JOIN public.preview_sessions ps ON ps.id = pr.session_id
      WHERE lower(ps.community) = lower(trim(_hoa_name))
  ...
  costs_m AS (
    ...
    FROM public.costs c
    JOIN households h ON h.normalized_address = c.normalized_address
    WHERE c.deleted_at IS NULL
      AND (lower(coalesce(c.cost_kind, '')) = 'monthly_plan'
  ...
  costs_sc AS (
    ...
    JOIN households h ON h.normalized_address = c.normalized_address
    WHERE c.deleted_at IS NULL
      AND lower(coalesce(c.cost_kind, '')) = 'service_call'
    GROUP BY c.vendor_id
  ),
  costs_hourly AS (
    SELECT c.vendor_id,
    ...
    JOIN households h ON h.normalized_address = c.normalized_address
    WHERE c.deleted_at IS NULL
      AND lower(coalesce(c.cost_kind, '')) = 'hourly'
    GROUP BY c.vendor_id
  ),
  -- Community mode calculation: most frequent price point, ties broken by higher amount
  hoa_costs AS (
    SELECT c.vendor_id,
           round(c.amount::numeric, 0) AS amt_rounded,
           coalesce(c.unit, 'month') AS unit
    FROM public.costs c
    JOIN households h ON h.normalized_address = c.normalized_address
    WHERE c.deleted_at IS NULL
  ),
  cost_counts AS (
    SELECT vendor_id, unit, amt_rounded, count(*) AS freq
    FROM hoa_costs
    GROUP BY vendor_id, unit, amt_rounded
  ),
  community_mode AS (
    SELECT DISTINCT ON (vendor_id)
           vendor_id,
           amt_rounded::numeric AS community_amount,
           unit AS community_unit
    FROM cost_counts
    ORDER BY vendor_id, freq DESC, amt_rounded DESC
  ),
  community_samples AS (
    SELECT vendor_id, count(*)::int AS community_sample_size
    FROM hoa_costs
    GROUP BY vendor_id
  ),
  ca AS (
  ...
  SELECT
    ...
    CASE 
      ...
      WHEN lower(v.category) IN ('plumbing', 'electrical', 'pet grooming', 'house cleaning', 'mobile tire repair', 'appliance repair') THEN
        CASE WHEN sc.service_call_avg IS NOT NULL THEN 'per Visit' ELSE 'See in Reviews' END
    ...
    CASE 
      ...
      WHEN lower(v.category) IN ('plumbing', 'electrical', 'pet grooming', 'house cleaning', 'mobile tire repair', 'appliance repair') THEN sc.service_call_avg
      WHEN lower(v.category) = 'handyman' THEN ch.hourly_avg
      ELSE NULL
    END AS avg_cost_amount,
    -- New community mode and market price fields
    comm_mode.community_amount,
    comm_mode.community_unit,
    comm_samples.community_sample_size,
    vmp.amount AS market_amount,
    vmp.unit AS market_unit
  ...
  LEFT JOIN community_mode comm_mode ON comm_mode.vendor_id = v.id
  LEFT JOIN community_samples comm_samples ON comm_samples.vendor_id = v.id
  LEFT JOIN public.vendor_market_prices vmp ON vmp.vendor_id = v.id
