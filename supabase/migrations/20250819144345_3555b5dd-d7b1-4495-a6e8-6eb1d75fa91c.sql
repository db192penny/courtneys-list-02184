...
BEGIN
  ...
  IF NEW.email_confirmed_at IS NOT NULL AND NOT EXISTS(SELECT 1 FROM public.users WHERE id = NEW.id) THEN
    BEGIN
      ...
      normalized_addr := public.normalize_address(user_address);
...
BEGIN
  ...
  IF OLD.address IS DISTINCT FROM NEW.address THEN
    ...
    NEW.street_name := normalized_addr;
...
BEGIN
  ...
  LOOP
    ...
    IF detected_hoa != 'Unknown Community' THEN
      ...
      UPDATE public.users 
      SET street_name = normalized_addr
      WHERE id = user_record.id AND street_name != normalized_addr;
...
-- ============================================================================
-- PHASE 4: Address Validation and Normalization Consistency
-- ============================================================================

-- Ensure the normalize_address function is consistent and handles edge cases
...
-- Create helper function to detect Boca Bridges addresses
CREATE OR REPLACE FUNCTION public.is_boca_bridges_address(_addr text)
RETURNS boolean
LANGUAGE sql
IMMUTABLE
SET search_path = 'public'
AS $function$
  SELECT lower(coalesce(_addr, '')) LIKE '%boca bridges%' OR 
         lower(coalesce(_addr, '')) LIKE '%33496%' OR
         lower(coalesce(_addr, '')) LIKE '%boca raton%';
$function$;

-- ============================================================================
-- Execute the backfill to fix current issues immediately
-- ============================================================================

-- Run the backfill function to fix the 7 missing users
SELECT * FROM public.backfill_missing_household_hoa();