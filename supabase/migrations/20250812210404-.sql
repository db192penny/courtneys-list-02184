-- 1) Remove notes/personal notes fields
DO $$ BEGIN
  IF EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_schema = 'public' AND table_name = 'costs' AND column_name = 'notes'
  ) THEN
    ALTER TABLE public.costs DROP COLUMN notes;
  END IF;
END $$;
...
DO $$ BEGIN
  IF EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_schema = 'public' AND table_name = 'home_vendors' AND column_name = 'my_comments'
  ) THEN
    ALTER TABLE public.home_vendors DROP COLUMN my_comments;
  END IF;
END $$;

-- 2) Enforce unique vendors per community (case-insensitive)
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_indexes WHERE schemaname = 'public' AND indexname = 'vendors_unique_name_community'
  ) THEN
    CREATE UNIQUE INDEX vendors_unique_name_community ON public.vendors (lower(name), lower(community));
  END IF;
END $$;

-- 3) Add unique index to enable upsert for home_vendors (user_id, vendor_id)
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_indexes WHERE schemaname = 'public' AND indexname = 'home_vendors_unique_user_vendor'
  ) THEN
    CREATE UNIQUE INDEX home_vendors_unique_user_vendor ON public.home_vendors (user_id, vendor_id);
  END IF;
END $$;

-- 4) Tighten RLS: regular users cannot update/delete vendors (admins already can)
DO $$ BEGIN
  IF EXISTS (
    ...
    DROP POLICY "Users can update their own vendors" ON public.vendors;
  END IF;
  IF EXISTS (
    ...
    DROP POLICY "Users can delete their own vendors" ON public.vendors;
  END IF;
END $$;

-- 5) Update list_vendor_stats to also return typical_cost for UI fallbacks
