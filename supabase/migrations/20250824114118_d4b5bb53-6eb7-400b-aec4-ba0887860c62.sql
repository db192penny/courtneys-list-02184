-- COMPREHENSIVE POINT SYSTEM FIX
-- This migration removes duplicate triggers, fixes validation issues, and corrects all user points

-- Step 1: Remove duplicate and problematic triggers
DROP TRIGGER IF EXISTS trg_vendor_points_trigger ON public.vendors;
DROP TRIGGER IF EXISTS trg_cost_points_trigger ON public.costs;
DROP TRIGGER IF EXISTS trg_review_points_trigger ON public.reviews;
DROP TRIGGER IF EXISTS trg_rate_vendor_points_trigger ON public.reviews;
DROP TRIGGER IF EXISTS validate_user_points_trigger ON public.users;

-- Drop old functions that may have duplicate logic
DROP FUNCTION IF EXISTS public.trg_vendor_points();
DROP FUNCTION IF EXISTS public.trg_cost_points();
DROP FUNCTION IF EXISTS public.trg_review_points_with_history();
DROP FUNCTION IF EXISTS public.trg_rate_vendor_points_update();
DROP FUNCTION IF EXISTS public.trg_rate_vendor_points();
DROP FUNCTION IF EXISTS public.validate_user_points_consistency();

-- Step 2: Ensure we have the correct triggers in place
-- Check if the correct triggers exist, if not create them

-- Vendor submission points trigger
CREATE OR REPLACE FUNCTION public.trg_vendor_submission_points()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public'
AS $function$
BEGIN
  -- Award 5 points for vendor submission
  UPDATE public.users 
  SET points = COALESCE(points, 0) + 5
  WHERE id = NEW.created_by;
  
  -- Log to point history
  INSERT INTO public.user_point_history (
    user_id, 
    activity_type, 
    points_earned, 
    description, 
    related_id
  ) VALUES (
    NEW.created_by,
    'vendor_submission',
    5,
    'Submitted vendor: ' || NEW.name,
    NEW.id
  );
  
  RETURN NEW;
END;
$function$;

-- Create vendor trigger if it doesn't exist
DROP TRIGGER IF EXISTS trg_vendor_submission_points_trigger ON public.vendors;
CREATE TRIGGER trg_vendor_submission_points_trigger
  AFTER INSERT ON public.vendors
  FOR EACH ROW
  EXECUTE FUNCTION public.trg_vendor_submission_points();

-- Review points trigger (fixed to award 5 points per review, not per vendor)
CREATE OR REPLACE FUNCTION public.trg_review_submission_points()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public'
AS $function$
BEGIN
  -- Award 5 points for each review submission
  UPDATE public.users 
  SET points = COALESCE(points, 0) + 5
  WHERE id = NEW.user_id;
  
  -- Log to point history
  INSERT INTO public.user_point_history (
    user_id, 
    activity_type, 
    points_earned, 
    description, 
    related_id
  ) VALUES (
    NEW.user_id,
    'review_submission',
    5,
    'Reviewed vendor with ' || NEW.rating || ' stars',
    NEW.id
  );
  
  RETURN NEW;
END;
$function$;

-- Create review trigger if it doesn't exist
DROP TRIGGER IF EXISTS trg_review_submission_points_trigger ON public.reviews;
CREATE TRIGGER trg_review_submission_points_trigger
  AFTER INSERT ON public.reviews
  FOR EACH ROW
  EXECUTE FUNCTION public.trg_review_submission_points();

-- Cost submission points trigger
-- Check if we already have the correct cost trigger
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.triggers 
    WHERE trigger_name = 'trg_cost_submission_points_trigger' 
    AND event_object_table = 'costs'
  ) THEN
    -- Create the trigger
    CREATE TRIGGER trg_cost_submission_points_trigger
      AFTER INSERT ON public.costs
      FOR EACH ROW
      EXECUTE FUNCTION public.trg_cost_points_with_history();
  END IF;
END $$;

-- Step 3: Clean up any system correction history from previous partial fixes
DELETE FROM public.user_point_history 
WHERE activity_type = 'system_correction' 
AND description LIKE '%Fixed incorrect points%';

-- Step 4: Recalculate and fix all user points based on actual activities
DO $$
DECLARE
  user_record record;
  calculated_points integer;
  vendor_count integer;
  review_count integer;
  cost_count integer;
BEGIN
  FOR user_record IN 
    SELECT u.id, u.email, u.points, u.name
    FROM public.users u
  LOOP
    -- Count actual activities for this user
    SELECT 
      COALESCE(COUNT(v.id), 0),
      COALESCE(COUNT(r.id), 0),
      COALESCE(COUNT(c.id), 0)
    INTO vendor_count, review_count, cost_count
    FROM public.users u
    LEFT JOIN public.vendors v ON v.created_by = u.id
    LEFT JOIN public.reviews r ON r.user_id = u.id
    LEFT JOIN public.costs c ON c.created_by = u.id
    WHERE u.id = user_record.id;
    
    -- Calculate correct points: 5 for joining + 5 per vendor + 5 per review + 5 per cost
    calculated_points := 5 + (vendor_count * 5) + (review_count * 5) + (cost_count * 5);
    
    -- Fix the points if they don't match
    IF COALESCE(user_record.points, 0) != calculated_points THEN
      UPDATE public.users 
      SET points = calculated_points 
      WHERE id = user_record.id;
      
      -- Log the correction in point history
      INSERT INTO public.user_point_history (
        user_id,
        activity_type,
        points_earned,
        description
      ) VALUES (
        user_record.id,
        'system_correction',
        calculated_points - COALESCE(user_record.points, 0),
        'System correction: Fixed points to ' || calculated_points || 
        ' (5 join + ' || (vendor_count * 5) || ' vendor + ' || 
        (review_count * 5) || ' review + ' || (cost_count * 5) || ' cost points)'
      );
    END IF;
  END LOOP;
END $$;

-- Step 5: Add a final verification function (admin-only) for future use
CREATE OR REPLACE FUNCTION public.verify_point_system_health()
RETURNS TABLE(
  total_users integer,
  users_with_correct_points integer,
  users_with_discrepancies integer,
  total_discrepancy_amount integer
)
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public'
AS $function$
DECLARE
  total_count integer := 0;
  correct_count integer := 0;
  discrepancy_count integer := 0;
  total_discrepancy integer := 0;
  user_record record;
  calculated_points integer;
BEGIN
  IF NOT public.is_admin() THEN
    RAISE EXCEPTION 'Not authorized - admin only';
  END IF;

  FOR user_record IN 
    SELECT u.id, u.points
    FROM public.users u
  LOOP
    total_count := total_count + 1;
    
    -- Calculate what points should be
    SELECT 
      5 + 
      (COALESCE((SELECT COUNT(*) FROM public.vendors WHERE created_by = user_record.id), 0) * 5) +
      (COALESCE((SELECT COUNT(*) FROM public.reviews WHERE user_id = user_record.id), 0) * 5) +
      (COALESCE((SELECT COUNT(*) FROM public.costs WHERE created_by = user_record.id), 0) * 5)
    INTO calculated_points;
    
    IF COALESCE(user_record.points, 0) = calculated_points THEN
      correct_count := correct_count + 1;
    ELSE
      discrepancy_count := discrepancy_count + 1;
      total_discrepancy := total_discrepancy + ABS(COALESCE(user_record.points, 0) - calculated_points);
    END IF;
  END LOOP;
  
  RETURN QUERY SELECT total_count, correct_count, discrepancy_count, total_discrepancy;
END;
$function$;