...
create policy if not exists "Public can view community photos"
  on storage.objects for select
  using (bucket_id = 'community-photos');

-- HOA admins can insert under their HOA folder (first path segment must match their HOA)
create policy if not exists "HOA admins can upload community photos"
...
create policy if not exists "HOA admins can update community photos"
...
create policy if not exists "HOA admins can delete community photos"
...
create policy if not exists "Anyone can read community assets"
  on public.community_assets for select
  using (true);

-- HOA admins can insert their own HOA asset row
create policy if not exists "HOA admins can insert community asset"
...
create policy if not exists "HOA admins can update community asset"
  on public.community_assets for update
  using (
    is_user_hoa_admin() and exists (
      select 1 from get_my_oa() g where lower(g.hoa_name) = lower(hoa_name)
    )
  )
  with check (
    is_user_hoa_admin() and exists (
...
create policy if not exists "HOA admins can delete community asset"
...
create trigger if not exists update_community_assets_updated_at
before update on public.community_assets
for each row execute function public.update_updated_at_column();

-- Function + trigger to set updated_by to current user id
...
create trigger if not exists set_community_assets_updated_by
