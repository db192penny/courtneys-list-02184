-- Fix home counting in list_vendor_stats to count distinct addresses instead of summing duplicates
CREATE OR REPLACE FUNCTION public.list_vendor_stats(_hoa_name text, _category text DEFAULT NULL::text, _sort_by text DEFAULT 'homes'::text, _limit integer DEFAULT 100, _offset integer DEFAULT 0)
 RETURNS TABLE(id uuid, name text, category text, homes_serviced integer, homes_pct numeric, hoa_rating numeric, hoa_rating_count integer, google_rating numeric, google_rating_count integer, avg_monthly_cost numeric, monthly_sample_size integer, service_call_avg numeric, service_call_sample_size integer, contact_info text, typical_cost numeric)
 LANGUAGE sql
