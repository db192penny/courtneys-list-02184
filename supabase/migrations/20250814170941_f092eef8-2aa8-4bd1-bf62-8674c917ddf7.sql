-- Add admin management columns to costs table
ALTER TABLE public.costs 
ADD COLUMN admin_modified BOOLEAN DEFAULT FALSE,
ADD COLUMN admin_modified_by UUID,
ADD COLUMN admin_modified_at TIMESTAMP WITH TIME ZONE,
ADD COLUMN deleted_at TIMESTAMP WITH TIME ZONE,
ADD COLUMN deleted_by UUID;

-- Create admin RLS policies for costs
CREATE POLICY "Admins can read all costs" 
ON public.costs 
FOR SELECT 
USING (is_admin());

CREATE POLICY "Admins can update all costs" 
ON public.costs 
FOR UPDATE 
USING (is_admin())
WITH CHECK (is_admin());

CREATE POLICY "Admins can delete all costs" 
ON public.costs 
FOR DELETE 
USING (is_admin());

-- Update list_vendor_stats function to exclude soft-deleted costs
