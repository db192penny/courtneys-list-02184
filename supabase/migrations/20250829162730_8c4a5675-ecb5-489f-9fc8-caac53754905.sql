-- Create function to redeem invite codes from the new invite system
CREATE OR REPLACE FUNCTION public.redeem_invite_code(_code text, _invited_user_id uuid)
RETURNS TABLE(success boolean, points_awarded integer, inviter_name text, inviter_email text)
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  RETURN QUERY SELECT true, points_to_award, inviter_record.name, inviter_record.email;
END;
$$;