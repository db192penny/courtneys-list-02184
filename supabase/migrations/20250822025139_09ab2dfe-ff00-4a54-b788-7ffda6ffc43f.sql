-- Restore the original homes counting logic in list_vendor_stats function
...
AS $function$
  ...
  homes AS (
    ...
    FROM (
      ...
      -- Costs: count homes where residents shared costs for the vendor
      SELECT c.vendor_id, c.normalized_address
      FROM public.costs c
      JOIN public.household_hoa hh ON hh.normalized_address = c.normalized_address
      WHERE lower(hh.hoa_name) = lower(trim(_hoa_name))
        AND c.deleted_at IS NULL
      
      UNION
      
      -- Vendor creators: count homes where residents submitted the vendor
      ...
      JOIN public.household_hoa hh ON hh.normalized_address = public.normalize_address(u.address)
      WHERE lower(hh.hoa_name) = lower(trim(_hoa_name))
        AND lower(v.community) = lower(trim(_hoa_name))
  ...
  SELECT
    ...
    CASE 
      ...
      WHEN lower(v.category) IN ('plumbing', 'electrical', 'pet grooming', 'house cleaning') THEN
        CASE WHEN sc.service_call_avg IS NOT NULL THEN 'per Visit' ELSE 'See in Reviews' END
    ...
    CASE 
      ...
      WHEN lower(v.category) IN ('plumbing', 'electrical', 'pet grooming', 'house cleaning') THEN sc.service_call_avg
