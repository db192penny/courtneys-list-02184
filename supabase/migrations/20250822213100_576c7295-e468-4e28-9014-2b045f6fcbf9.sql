-- Update list_vendor_stats function to include Mobile Tire Repair in service call category group
...
AS $function$
  ...
  SELECT
    ...
    CASE 
      ...
      WHEN lower(v.category) IN ('plumbing', 'electrical', 'pet grooming', 'house cleaning', 'mobile tire repair') THEN
        CASE WHEN sc.service_call_avg IS NOT NULL THEN 'per Visit' ELSE 'See in Reviews' END
    ...
    CASE 
      ...
      WHEN lower(v.category) IN ('plumbing', 'electrical', 'pet grooming', 'house cleaning', 'mobile tire repair') THEN sc.service_call_avg
